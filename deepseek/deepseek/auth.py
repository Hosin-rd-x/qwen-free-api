...
if __name__ == "__main__":
    ...
    from .pool_accounts import accounts_dir

    parser = argparse.ArgumentParser(
        description="Sign in to chat.deepseek.com and save a session."
    )
    parser.add_argument(
        "--account", "-a", default=None, metavar="NAME",
        help="save the session as session/accounts/NAME.json (multi-account "
             "pool). Repeat with different names to add more accounts.",
    )
    parser.add_argument(
    ...
    if args.list:
        ...
        legacy = d.parent / "session.json"
        if legacy.is_file():
            print(f"- default   ({legacy})")
        for p in found:
            print(f"- {p.stem:10s} ({p})")
        if not found and not legacy.is_file():
            print("No accounts yet — run: python -m deepseek.auth --account main")
        raise SystemExit(0)

    if args.account:
        target = accounts_dir() / f"{args.account}.json"
        s = login()
        s.save(target)
        print(f"[auth] account '{args.account}' saved -> {target}")
        print(f"[auth] token {s.token[:10]}... ({len(s.cookies)} cookies)")
        print("[auth] the running server picks it up automatically "
              "(or restart it).")
    else:
        s = login()
        print(f"[auth] captured token {s.token[:10]}... "
